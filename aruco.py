...
if __name__ == '__main__':
    ...
    for i in range(grid_cols):
        for j in range(grid_rows):
            ...
            # step3 获得marker内容并赋值 - 更新为新API
            tmp_marker = np.zeros((grid_size, grid_size), dtype=np.uint8)
            tmp_marker = cv2.aruco.generateImageMarker(dictionary, tmp_index, grid_size, tmp_marker, marker_boarder)
            markerImg[start_x:start_x + grid_size, start_y:start_y + grid_size] = tmp_marker

    # step4 保存marker
    cv2.imwrite("arucoBoard.png", markerImg)
    print("ArUco板已生成，文件名: arucoBoard.png")
